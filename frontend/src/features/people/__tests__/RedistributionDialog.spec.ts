...
describe('people redistribution workflow', () => {
  ...
  it('requires a nonblank unavailability reason', async () => {
    ...
    await wrapper.get('[data-testid="save-unavailable"]').trigger('click')

    expect(wrapper.text()).toContain('请输入不可参与原因')
  ...
  it('separates executing tasks from pending redistribution preview', async () => {
    ...
    await vi.waitFor(() => expect(wrapper.text()).toContain('OPS-PENDING'))
    expect(wrapper.text()).toContain('需手工调整')
    expect(wrapper.text()).toContain('OPS-RUNNING')
  })
  ...
  it('keeps per-task failures visible and links to failed task detail', async () => {
    ...
    await vi.waitFor(() => expect(wrapper.text()).toContain('没有可用候选人'))
    expect(wrapper.get('a[href="/tasks/failed-1"]').attributes('href'))
      .toBe('/tasks/failed-1')
  })
})
