...
test('final assignee calls and completes a task with actual duration', async ({
  ...
  await expect(operatorSession.page.locator('[role="dialog"]').getByText('执行中', { exact: true })).toBeVisible()
  ...
  await expect(operatorSession.page.locator('[role="dialog"]').getByText('已完成', { exact: true })).toBeVisible()
  await expect(operatorSession.page.locator('[role="dialog"]').getByText('60 分钟', { exact: true })).toBeVisible()

  await developer.page.goto(`/tasks/${created.id}`)
  await expect(developer.page.locator('[role="dialog"]').getByText('已完成', { exact: true })).toBeVisible()
